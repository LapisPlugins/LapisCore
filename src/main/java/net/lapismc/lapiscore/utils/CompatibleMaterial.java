...
/**
 * Class made by CryptoMorin and adapted for LapisCore by Dart2112
 * The original code can be found at https://github.com/CryptoMorin/XMaterial/blob/master/XMaterial.java
...
public enum CompatibleMaterial {
     ...
     */
    public static final HashMap<CompatibleMaterial, CompatibleMaterial> DUPLICATED = new HashMap<CompatibleMaterial, CompatibleMaterial>() {{
        put(CompatibleMaterial.MELON, CompatibleMaterial.MELON_SLICE);
        put(CompatibleMaterial.CARROT, CompatibleMaterial.CARROTS);
        put(CompatibleMaterial.POTATO, CompatibleMaterial.POTATOES);
        put(CompatibleMaterial.BEETROOT, CompatibleMaterial.BEETROOTS);
        put(CompatibleMaterial.BROWN_MUSHROOM, CompatibleMaterial.BROWN_MUSHROOM_BLOCK);
        put(CompatibleMaterial.BRICK, CompatibleMaterial.BRICKS);
        put(CompatibleMaterial.RED_MUSHROOM, CompatibleMaterial.RED_MUSHROOM_BLOCK);
        put(CompatibleMaterial.MAP, CompatibleMaterial.FILLED_MAP);
        put(CompatibleMaterial.NETHER_BRICK, CompatibleMaterial.NETHER_BRICKS);
    }};
    public static final CompatibleMaterial[] VALUES = CompatibleMaterial.values();
    public static final String[] LEGACY_VALUES = Arrays.stream(VALUES).map(m -> m.legacy).flatMap(Arrays::stream).toArray(String[]::new);
    private static final HashMap<String, CompatibleMaterial> CACHED_SEARCH = new HashMap<>();
    private static MinecraftVersion version;
    private static Boolean isNewVersion;
    private final byte data;
    private final String[] legacy;

    CompatibleMaterial(int data, String... legacy) {
        this.data = (byte) data;
        this.legacy = legacy;
    }
     ...
     */
    private static CompatibleMaterial requestOldXMaterial(String name, byte data) {
        CompatibleMaterial cached = CACHED_SEARCH.get(name + "," + data);

        if (cached != null) return cached;
        Optional<CompatibleMaterial> search = data == -1 ? Arrays.stream(CompatibleMaterial.VALUES).filter(mat -> mat.matchAnyLegacy(name)).findFirst()
                : Arrays.stream(CompatibleMaterial.VALUES).filter(mat -> mat.matchAnyLegacy(name) && mat.data == data).findFirst();

        if (search.isPresent()) {
            CompatibleMaterial found = search.get();
    ...
    public static boolean contains(String name) {
        ...
        return Arrays.stream(CompatibleMaterial.VALUES).anyMatch(mat -> mat.name().equals(formatted));
    }
     ...
     * @see #matchXMaterial(String, byte)
     */
    public static CompatibleMaterial matchXMaterial(Material material) {
     ...
     * @see #matchXMaterial(String, byte)
     */
    public static CompatibleMaterial matchXMaterial(String name) {
    ...
    public static CompatibleMaterial matchXMaterial(ItemStack item) {
     ...
     */
    public static CompatibleMaterial matchXMaterial(String name, byte data) {
        Validate.notEmpty(name, "Material name cannot be null or empty");
        name = format(name);
     ...
     */
    public static CompatibleMaterial matchXMaterial(int id, byte data) {
        // Looping through Material.values() will take longer.
        return Arrays.stream(CompatibleMaterial.VALUES).filter(mat -> mat.getId() == id && mat.data == data).findFirst().orElse(null);
    }
     ...
     */
    private static CompatibleMaterial requestDuplicatedXMaterial(String name, byte data) {
        Validate.notEmpty(name, "Material name cannot be null or empty");
        CompatibleMaterial mat = requestOldXMaterial(name, data);
     ...
     */
    public static CompatibleMaterial getNewXMaterialIfDuplicated(String name) {
        Validate.notEmpty(name, "Material name cannot be null or empty");
     ...
     * @see #matchXMaterial(String, byte)
     */
    public static CompatibleMaterial getXMaterialIfDuplicated(String name) {
        Validate.notEmpty(name, "Material name cannot be null or empty");
        String formatted = format(name);
        Optional<Map.Entry<CompatibleMaterial, CompatibleMaterial>> mat = DUPLICATED.entrySet().stream().filter(m
                -> m.getKey().name().equals(formatted) || m.getValue().name().equals(formatted)).findFirst();

        if (mat.isPresent()) {
            Map.Entry<CompatibleMaterial, CompatibleMaterial> found = mat.get();
     ...
     * @see #matchXMaterial(String, byte)
     */
    public CompatibleMaterial getXMaterialIfDuplicated() {
        Optional<Map.Entry<CompatibleMaterial, CompatibleMaterial>> mat = DUPLICATED.entrySet().stream().filter(m -> m.getKey() == this).findFirst();
     ...
     * @see #parseMaterial()
     */
    public CompatibleMaterial suggestOldMaterialIfNew() {
        ...
        for (int i = this.legacy.length - 1; i >= 0; i--) {
            ...
            CompatibleMaterial mat = matchXMaterial(parseLegacyVersionMaterialName(legacyName), this.data);
